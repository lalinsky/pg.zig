...
const TLSStream = struct {
    ...
    pub fn connect(allocator: Allocator, rt: *zio.Runtime, opts: Conn.Opts, ctx_: ?*openssl.SSL_CTX) !Stream {
        _ = allocator;
        _ = rt;
        _ = opts;
        _ = ctx_;
        ...
        // const plain = try PlainStream.connect(allocator, rt, opts, null);
...
const PlainStream = struct {
    ...
    pub fn connect(allocator: Allocator, rt: *zio.Runtime, opts: Conn.Opts, _: anytype) !PlainStream {
        _ = allocator;
        const stream = blk: {
            const host = opts.host orelse DEFAULT_HOST;
            if (host.len > 0 and host[0] == '/') {
                const addr = try zio.net.UnixAddress.init(host);
                break :blk try addr.connect(rt, .{});
            }
            ...
            if (zio.net.IpAddress.parseIp4(host, port)) |addr| {
                ...
                break :blk try hostname.connect(rt, port, .{});
            }
        };
